...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, darkMode, setDarkMode, lang, setLang, onSearchSelect }) => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 flex flex-col md:flex-row print:block print:min-h-0 ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-gray-50 text-gray-900'}`}>
      {/* Mobile Header */}
      <header className={`md:hidden p-4 flex flex-col sticky top-0 z-50 transition-all duration-300 no-print ${darkMode ? 'bg-slate-900 border-b border-slate-800' : 'bg-blue-700 text-white shadow-md'}`}>
      ...
      {/* Main Content Area */}
      <main className="flex-1 h-screen overflow-y-auto relative custom-scrollbar print:h-auto print:overflow-visible print:block">
        <div className="max-w-6xl mx-auto p-4 md:p-8 print:p-0 print:max-w-none">
          {/* Desktop Top Header Bar */}
          <div className="hidden md:flex items-center justify-between mb-8 gap-6 no-print">
