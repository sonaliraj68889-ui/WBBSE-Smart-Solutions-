...
const SmartTutor: React.FC<SmartTutorProps> = ({ darkMode, lang, initialQuery, initialFile, onSaveSearch, onHome, onQuotaExceeded }) => {
  ...
  const startLiveSession = async () => {
    if (isLiveSession || liveStatus !== 'idle') {
      stopLiveSession();
      return;
    }
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (audioData && audioContextOutRef.current) {
              ...
              source.onended = () => {
                ...
                if (audioSourcesRef.current.size === 0 && liveStatus !== 'idle') {
                  setLiveStatus('listening');
                }
              };
            }
            ...
            if (message.serverContent?.interrupted) {
              audioSourcesRef.current.forEach(s => { try { s.stop(); s.disconnect(); } catch(e){} });
  ...
  const stopLiveSession = () => {
    if (liveSessionRef.current) {
      liveSessionRef.current.then((s: any) => { if(s && s.close) s.close(); }).catch(() => {});
        ...
        {liveStatus !== 'idle' && (
          <div className="absolute inset-x-0 top-0 z-10 p-4 md:p-6 animate-fadeIn">
            <div className={`p-4 md:p-5 rounded-3xl flex items-center justify-between border shadow-2xl backdrop-blur-xl ${darkMode ? 'bg-slate-900/90 border-indigo-500/30' : 'bg-white/90 border-indigo-200'}`}>
              <div className="flex items-center space-x-3 md:space-x-4">
                <div className={`flex space-x-1 items-end h-6 md:h-8 ${liveStatus === 'connecting' ? 'opacity-50' : ''}`}>
                  {[...Array(6)].map((_, i) => (
                    <div key={i} className={`w-1 md:w-1.5 bg-indigo-500 rounded-full shadow-[0_0_10px_rgba(99,102,241,0.5)] ${liveStatus === 'speaking' ? 'animate-bounce' : (liveStatus === 'listening' ? 'animate-pulse' : '')}`} style={{ height: `${30 + Math.random() * 70}%`, animationDelay: `${i * 150}ms` }}></div>
                  ))}
                </div>
                <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] text-indigo-500">
                  {liveStatus === 'connecting' ? (lang === 'hi' ? 'कनेक्ट हो रहा है...' : 'Connecting...') : 
                   liveStatus === 'speaking' ? (lang === 'hi' ? 'ट्यूटर बोल रहा है...' : 'Tutor is Speaking...') :
                   (lang === 'hi' ? 'ट्यूटर सुन रहा है...' : 'Tutor is Listening...')}
                </span>
              </div>
            ...
            <div className="flex items-center space-x-1.5">
              <button 
                ...
                className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all duration-500 shadow-md ${
                  liveStatus !== 'idle' ? 'bg-red-500 text-white animate-pulse scale-105 shadow-red-500/50' : (darkMode ? 'bg-slate-800 text-blue-400 hover:bg-slate-700 border border-slate-700' : 'bg-blue-50 text-blue-600 hover:bg-blue-100 border border-blue-100')
                }`}
                title={lang === 'hi' ? 'आवाज से बात करें' : 'Talk with Voice'}
              >
                <i className={`fa-solid ${liveStatus !== 'idle' ? 'fa-phone-slash' : 'fa-microphone'} text-base`}></i>
              </button>
              ...
              <div className="flex-1 relative group">
                <input 
                  type="text" 
                  placeholder={liveStatus !== 'idle' ? (lang === 'hi' ? 'बोलिए...' : 'Say something...') : t.diagramPrompt}
